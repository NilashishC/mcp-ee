...
- `mcp-ee.yaml` - This is the main definition file that `ansible-builder` will use to construct your EE image.
...
Open `mcp-ee.yaml` in your favorite text editor and review the configuration.
...
Before you build the EE, you must ensure `ansible-builder` can access all the collections you listed in your `mcp-ee.yaml` file.
...
This command uses your `mcp-ee.yaml` file to build an image and tags it as `mcp-ee:latest`
...
```bash
ansible-navigator run playbook.yml --execution-environment-image mcp-ee:latest
```
...
1. Tag your local image for the target registry (e.g., `quay.io/your-username/mcp-ee:latest`).

```bash
podman tag mcp-ee:latest quay.io/your-username/mcp-ee:latest
```

2. Push the tagged image to the target registry:

```bash
podman push quay.io/your-username/mcp-ee:latest
```
