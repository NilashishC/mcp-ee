...
- `ee68.yaml` - This is the main definition file that `ansible-builder` will use to construct your EE image.
...
Open `ee68.yaml` in your favorite text editor and review the configuration.
...
Before you build the EE, you must ensure `ansible-builder` can access all the collections you listed in your `ee68.yaml` file.
...
```bash
ansible-builder build --file ee68.yaml --tag ee68:latest --container-runtime podman
```

This command uses your `ee68.yaml` file to build an image and tags it as `ee68:latest`
...
```bash
ansible-navigator run playbook.yml --execution-environment-image ee68:latest
```
...
1. Tag your local image for the target registry (e.g., `quay.io/your-username/ee68:latest`).

```bash
podman tag ee68:latest quay.io/your-username/ee68:latest
```

2. Push the tagged image to the target registry:

```bash
podman push quay.io/your-username/ee68:latest
```
