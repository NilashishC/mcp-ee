...
- `mcp.yaml` - This is the main definition file that `ansible-builder` will use to construct your EE image.
...
Open `mcp.yaml` in your favorite text editor and review the configuration.
...
Before you build the EE, you must ensure `ansible-builder` can access all the collections you listed in your `mcp.yaml` file.
...
```bash
ansible-builder build --file mcp.yaml --tag mcp:latest --container-runtime podman
```

This command uses your `mcp.yaml` file to build an image and tags it as `mcp:latest`
...
```bash
ansible-navigator run playbook.yml --execution-environment-image mcp:latest
```
...
1. Tag your local image for the target registry (e.g., `quay.io/your-username/mcp:latest`).

```bash
podman tag mcp:latest quay.io/your-username/mcp:latest
```

2. Push the tagged image to the target registry:

```bash
podman push quay.io/your-username/mcp:latest
```
