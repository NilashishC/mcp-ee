...
- `eeX.yaml` - This is the main definition file that `ansible-builder` will use to construct your EE image.
...
Open `eeX.yaml` in your favorite text editor and review the configuration.
...
Before you build the EE, you must ensure `ansible-builder` can access all the collections you listed in your `eeX.yaml` file.
...
```bash
ansible-builder build --file eeX.yaml --tag eeX:latest --container-runtime podman
```

This command uses your `eeX.yaml` file to build an image and tags it as `eeX:latest`
...
```bash
ansible-navigator run playbook.yml --execution-environment-image eeX:latest
```
...
1. Tag your local image for the target registry (e.g., `quay.io/your-username/eeX:latest`).

```bash
podman tag eeX:latest quay.io/your-username/eeX:latest
```

2. Push the tagged image to the target registry:

```bash
podman push quay.io/your-username/eeX:latest
```
