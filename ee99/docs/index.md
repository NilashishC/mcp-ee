...
- `ee99.yaml` - This is the main definition file that `ansible-builder` will use to construct your EE image.
...
Open `ee99.yaml` in your favorite text editor and review the configuration.
...
Before you build the EE, you must ensure `ansible-builder` can access all the collections you listed in your `ee99.yaml` file.
...
```bash
ansible-builder build --file ee99.yaml --tag ee99:latest --container-runtime podman
```

This command uses your `ee99.yaml` file to build an image and tags it as `ee99:latest`
...
```bash
ansible-navigator run playbook.yml --execution-environment-image ee99:latest
```
...
1. Tag your local image for the target registry (e.g., `quay.io/your-username/ee99:latest`).

```bash
podman tag ee99:latest quay.io/your-username/ee99:latest
```

2. Push the tagged image to the target registry:

```bash
podman push quay.io/your-username/ee99:latest
```
