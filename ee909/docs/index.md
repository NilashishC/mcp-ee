...
- `ee909.yaml` - This is the main definition file that `ansible-builder` will use to construct your EE image.
...
Open `ee909.yaml` in your favorite text editor and review the configuration.
...
Before you build the EE, you must ensure `ansible-builder` can access all the collections you listed in your `ee909.yaml` file.
...
```bash
ansible-builder build --file ee909.yaml --tag ee909:latest --container-runtime podman
```

This command uses your `ee909.yaml` file to build an image and tags it as `ee909:latest`
...
```bash
ansible-navigator run playbook.yml --execution-environment-image ee909:latest
```
...
1. Tag your local image for the target registry (e.g., `quay.io/your-username/ee909:latest`).

```bash
podman tag ee909:latest quay.io/your-username/ee909:latest
```

2. Push the tagged image to the target registry:

```bash
podman push quay.io/your-username/ee909:latest
```
